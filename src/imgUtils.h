...
template<typename TMat>
inline std::vector<TMat> imSyntheticGeneration(const TMat& image, int translationOffset = 4)
{
...
// Returns a vector containing the original image followed by multiple synthetic images generated from the original
template<typename TMat>
inline std::vector<TMat> imSyntheticGenerationScaleAndTranslation(const TMat& image, int nScales, int translationSize, double minScale)
{
    double scaleJumps = (1 - minScale) / nScales;
    std::vector<TMat> synthImages;
